...
from settings_honkai import log, CONFIG_honkai, req_honkai


class Base_honkai(object):
    ...
    def get_header(self):
        header = {
            'User-Agent': CONFIG_honkai.WB_USER_AGENT,
            'Referer': CONFIG_honkai.OS_REFERER_URL,
...
class Roles_honkai(Base_honkai):
    def get_awards(self):
        response = {}
        try:
            response = req_honkai.to_python(req_honkai.request(
                'get', CONFIG_honkai.OS_REWARD_URL, headers=self.get_header()).text)
    ...
    def get_roles(self):
        ...
        try:
            response = req_honkai.to_python(req_honkai.request(
                'get', CONFIG_honkai.OS_ROLE_URL, headers=self.get_header()).text)
...
class Sign_honkai(Base_honkai):
    def __init__(self, cookies: str = None):
        super(Sign_honkai, self).__init__(cookies)
        self._region_name = ''
        self._uid = ''
        self._level = ''
        self._nick_name = ''
    ...
    def get_info(self):
        index = 0
        user_game_roles = Roles_honkai(self._cookie).get_roles()
        ...
        info_url = CONFIG_honkai.OS_INFO_URL
        try:
            response = req_honkai.request(
                'get', info_url, headers=self.get_header()).text
            return req_honkai.to_python(response)
        except Exception as e:
    ...
    def run(self):
        ...
        if info_list:
            ...
            awards = Roles_honkai(self._cookie).get_awards().get('data',{}).get('awards')
            uid = str(self._uid)#.replace(str(self._uid)[1:7], ' ▓ ▓ ▓ ▓ ▓ ▓ ▓ ', 1)
            ...
            data = {
                'act_id': CONFIG_honkai.OS_ACT_ID
            }

            try:
                response = req_honkai.to_python(req_honkai.request(
                    'post', CONFIG_honkai.OS_SIGN_URL, headers=self.get_header(),
    ...
    @property
    def message(self):
        return CONFIG_honkai.MESSAGE_TEMPLATE