...
class Notify_honkai(object):
    ...
    def __init__(self):
        # Custom Push Config
        self.PUSH_CONFIG = ''
        if os.getenv('PUSH_CONFIG'):
            self.PUSH_CONFIG = os.getenv('PUSH_CONFIG')
        # Discord Webhook
    ...
    def send(self, app='HSR Helper', status='', msg='', **kwargs):
        ...
        params = [('chat_id',chat),('text', app+status+"\n"+msg)]
        response = requests.get("https://api.telegram.org/bot"+api+"/sendMessage", params)
        log.info(response.content)        


if __name__ == '__main__':
    Notify_honkai().send(app='HSR Check-In Helper', status='Test Run', msg='Testing integration with social media APIs')